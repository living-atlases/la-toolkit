...
.PARAMETER Clean
Removes the image la-toolkit and kills all containers based on that image.
.PARAMETER ComposeForDevelop
Builds the image and runs docker-compose.
.PARAMETER Environment
The enviorment to build for (Develop or Release), defaults to Develop
.EXAMPLE
C:\PS> .\dockerTask.ps1 -Build
Build a Docker image named la-toolkit
#>

Param(
    [Parameter(Mandatory=$True,ParameterSetName="Compose")]
    [switch]$Compose,
    [Parameter(Mandatory=$True,ParameterSetName="ComposeForDevelop")]
    [switch]$ComposeForDevelop,
    [Parameter(Mandatory=$True,ParameterSetName="Build")]
    [switch]$Build,
    [Parameter(Mandatory=$True,ParameterSetName="Clean")]
    [switch]$Clean,
    [parameter(ParameterSetName="Compose")]
    [Parameter(ParameterSetName="ComposeForDevelop")]
