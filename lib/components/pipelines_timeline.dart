...
class _PipelinesTimelineState extends State<PipelinesTimeline> {
  ...
  Widget build(BuildContext context) {
    ...
    leftActions.add(
      _DrInput(cmd.drs, cmd.allDrs, (String value) {
        setState(() {
          cmd.drs = value;
          widget.onChange(cmd);
        });
          ...
          onPressed: () {
            ...
            setState(() {
              if (isShiftPressed) {
                if (lastClicked >= 0) {
                  ...
                  if (loopForward) {
                    for (int x = lastClicked; x < index; x++) {
                      cmd.steps.add(
                        PipelinesStepDesc.allStringList.elementAt(x),
                      );
                    }
                  } else {
                    for (int x = lastClicked; x > index; x--) {
                ...
                final String el = PipelinesStepDesc.allStringList.elementAt(
                  index,
                );
                cmd.steps.add(el);
              } else {
...
class _StepTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      child: Text(visible ? text : '', textAlign: TextAlign.right),
    );
  }
}
...
class _StepWidget extends StatelessWidget {
  ...
  static IndicatorStyle notSelectedIndicatorStyle = IndicatorStyle(
    width: PipelinesTimeline.iconSize,
    padding: PipelinesTimeline.stepIconPadding,
    iconStyle: IconStyle(color: Colors.grey.shade400, iconData: Icons.check),
  );
  static IndicatorStyle noIndicatorStyle = IndicatorStyle(
    width: PipelinesTimeline.iconSize,
    padding: const EdgeInsets.all(2),
    iconStyle: IconStyle(color: Colors.transparent, iconData: Icons.check),
  );
...
class GenericActionChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ActionChip(
      ...
      label: Text(
        text,
        style: TextStyle(fontSize: 16, color: isPressed ? Colors.white : null),
      ),
    );
  }
}
