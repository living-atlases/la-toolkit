...
class ServiceWidget extends StatelessWidget {
  const ServiceWidget({super.key, required this.serviceName, required this.collectoryFocusNode});
  ...
  Widget build(BuildContext context) {
    final TextStyle domainTextStyle = TextStyle(fontSize: 16, color: Theme.of(context).hintColor);
    return StoreConnector<AppState, _LAServiceViewModel>(
      ...
      return _LAServiceViewModel(
          ...
          onEditService: (LAService service) => store.dispatch(EditService(service)),
      ...
      final bool noDependsOrInUse = serviceDesc.depends == null ||
          (isHub ? vm.currentProject.parent! : vm.currentProject).getServiceE(serviceDesc.depends!).use;
      final bool withoutUrl = serviceDesc.withoutUrl;
      final bool visible = noDependsOrInUse && (!withoutUrl || servicesWithoutUrlButShow.contains(serviceName));
      ...
      final ValueNotifier<bool> domainSwitchController = ValueNotifier<bool>(false);
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                    ...
                    ListTile(
                        ...
                        title: Row(children: <Widget>[
                          ...
                          const SizedBox(
                            width: 10, // here put the desired space between the icon and the text
                          ),
                          if (optional)
                            Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                              Row(children: <Widget>[
                                Text('Use the ${serviceDesc.name} service?'),
                                Transform.scale(
                                    scale: 0.8,
                                    child: Switch(
                                      value: service.use,
                                      ...
                                      onChanged: (bool newValue) {
                                        currentProject.serviceInUse(serviceName, newValue);
                                        vm.onSaveProject(currentProject);
                                      },
                                    ))
                              ]),
                              const SizedBox(
                                height: 0, // here put the desired space between the icon and the text
                              ),
                              SizedBox(
                          ...
                          if (!optional) Text('${serviceDesc.name}: ${StringUtils.capitalize(serviceDesc.desc)}')
                        ]),
                        trailing: Row(mainAxisSize: MainAxisSize.min, children: <Widget>[
                          if (serviceDesc.repository != null)
                            Tooltip(
                                ...
                                child: IconButton(
                                    onPressed: () async {
                                      await launchUrl(Uri.parse(serviceDesc.repository!));
                                    },
                                    icon: const Icon(Icons.code))),
                          if (serviceDesc.sample != null && serviceDesc.repository != null) const SizedBox(width: 8),
                          if (serviceDesc.sample != null)
                            serviceDesc.name == branding
                                ? HelpIcon(wikipage: 'Styling-the-web-app')
                                : HelpIcon.url(url: serviceDesc.sample!, tooltip: 'See a similar service in production')
                        ])),
                    if (!withoutUrl && (!optional || service.use))
                      Row(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                        ...
                        if (!withoutUrl)
                          Container(
                              padding: EdgeInsets.fromLTRB(0, 0, usesSubdomain ? 0 : 0, 0),
                              child: Text("http${vm.currentProject.useSSL ? 's' : ''}://", style: domainTextStyle)),
                        if (!withoutUrl && usesSubdomain)
                          _createSubUrlField(service, serviceDesc, vm, 'Invalid subdomain.'),
                        if (!withoutUrl && usesSubdomain) Text('.$domain/', style: domainTextStyle),
                        if (!withoutUrl && usesSubdomain && !serviceDesc.forceSubdomain)
                          _createPathField(service, serviceDesc, vm, 'Invalid path.'),
                        if (!withoutUrl && !usesSubdomain) Text('$domain/', style: domainTextStyle),
                        if (!withoutUrl && !usesSubdomain)
                          _createSubUrlField(service, serviceDesc, vm, 'Invalid path.'),
                        if (!withoutUrl && !usesSubdomain) Text('/', style: domainTextStyle),
  ...
  Widget _createSubUrlField(LAService service, LAServiceDesc serviceDesc, _LAServiceViewModel vm, String error) {
    return _wrapField(
        child: GenericTextFormField(
            initialValue: service.suburl,
            focusNode: service.nameInt == collectory ? collectoryFocusNode : null,
            isCollapsed: true,
  ...
  Widget _createPathField(LAService service, LAServiceDesc serviceDesc, _LAServiceViewModel vm, String error) {
    return _wrapField(
        child: GenericTextFormField(
            initialValue: service.iniPath,
            focusNode: service.nameInt == collectory && !service.usesSubdomain ? collectoryFocusNode : null,
            isCollapsed: true,
            contentPadding: const EdgeInsets.only(bottom: 4),
            regexp: LARegExp.subdomain,
            allowEmpty: true,
...
class _LAServiceViewModel {
  _LAServiceViewModel({required this.currentProject, required this.onEditService, required this.onSaveProject});
