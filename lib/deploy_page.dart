  ...
  Widget build(BuildContext context) {
    return StoreConnector<AppState, _DeployViewModel>(
      ...
      builder: (BuildContext context, _DeployViewModel vm) {
        ...
        final VoidCallback? onTap = cmd.deployServices.isEmpty ? null : () => vm.onDeployProject(vm.project, cmd);
        ...
        final bool onlyDocker = vm.project.isDockerClusterConfigured() &&
            const ListEquality<String>().equals(cmd.limitToServers, dockerServers);
        selectedVersions.addAll(vm.project.getServiceDeployReleases(onlyDocker));
        final List<MigrationNotesDesc> migrationNotes =
            DependenciesManager.getMigrationNotes(_servicesToDeploy, selectedVersions);
            ...
            child: Scaffold(
                ...
                appBar: LAAppBar(
                    ...
                    actions: <Widget>[
                      ...
                      IconButton(
                          icon: const Tooltip(
                              message: 'Cancel the deploy', child: Icon(Icons.close, color: Colors.white)),
                      ...
                      children: <Widget>[
                        ...
                        Expanded(
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                              ...
                              const ListTile(
                                  title: Text('Select which services you want to deploy:',
                                ...
                                ListTile(
                                    ...
                                    trailing: Switch(
                                        ...
                                        onChanged: (bool value) => setState(() {
                                              if (value) {
                                                cmd = cmd.copyWith(limitToServers: dockerServers);
                                              } else {
                                                final List<String> newServers = cmd.limitToServers
                                                    .where((String s) => !dockerServers.contains(s))
                                                    .toList();
                                                cmd = cmd.copyWith(limitToServers: newServers);
                                              }
                                              vm.onSaveDeployCmd(cmd);
                                            }))),
                              ListTile(
                              ...
                              if (advanced)
                                ServerSelector(
                                    ...
                                    modalTitle: 'Choose some servers if you want to limit the deploy to them',
                                    ...
                                    onChange: (List<String> limitToServers) => setState(() {
                                          cmd = cmd.copyWith(limitToServers: limitToServers);
                                          vm.onSaveDeployCmd(cmd);
                                        })),
                              if (advanced)
                                TagsSelector(
                                    ...
                                    selectorKey: GlobalKey<FormFieldState<dynamic>>(),
                                    tags: TagsConstants.getTagsFor(vm.project.alaInstallRelease),
                              ...
                              if (advanced)
                                TipsCard(text: "This project is generated in the '${vm.project.dirName}' directory."),
                              Column(
                                  children: migrationNotes
                                      .map((MigrationNotesDesc m) => AlertCard(
                                          message: m.text,
                                          color: Colors.grey.shade100,
                                          action: () => <Future<bool>>{launchUrl(Uri.parse(m.url))},
...
class _DeployViewModel {
  ...
  bool operator ==(Object other) =>
      ...
      other is _DeployViewModel && runtimeType == other.runtimeType && project == other.project && cmd == other.cmd;
