...
class HostsServicesChecks {
  ...
  void setUrls(
    ...
    List<String> urls,
    String name,
    List<String> serversIds,
    bool full,
  ) {
    final List<String> list = sd.serverId != null
        ? <String>[sd.serverId!]
        : <String>[];
    ...
    for (final String serverId in full ? serversIds : list) {
      ...
      for (final String url in urls) {
        final HostServiceCheck ch = hChecks.values.firstWhere(
          (HostServiceCheck ch) =>
              ch.type == ServiceCheckType.url && ch.args == url,
          orElse: () {
            final HostServiceCheck ch = HostServiceCheck(
              name: name,
              type: ServiceCheckType.url,
              args: url,
            );
            hChecks[ch.id] = ch;
            return ch;
    ...
    if (deps != null) {
      BasicService.toCheck(deps).forEach((BasicService dep) {
        for (final num tcp in dep.tcp) {
          ...
          for (final String serverId
              in full && dep.reachableFromOtherServers
                  ? serversIds
                  : <String>[sd.serverId!]) {
            final Map<String, HostServiceCheck> hChecks = _getServiceCheck(
              serverId,
            );
            final HostServiceCheck ch = hChecks.values.firstWhere(
              (HostServiceCheck ch) =>
