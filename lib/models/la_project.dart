...
class LAProject implements IsJsonSerializable<LAProject> {
  ...
  factory LAProject.fromObject(
    ...
    for (final LAServiceDesc serviceDesc in LAServiceDesc.list(false)) {
      ...
      // only the service name silently lost that, so an inventory that had
      // turned biocache-store off came back with it on -- which then turned
      // pipelines off. The service name is still tried as a fallback, for
      // inventories generated before that mapping.
      //
      // ala_bie -> species is deliberately NOT mapped here: old real inventories
      // (Canadensys) carry LA_use_species: false while clearly deploying the bie
      // (hostname and url are set), and they must keep importing as they did.
      final Map<String, String> useKeyAliases = <String, String>{
        'cas': 'CAS',
      ...
      final bool useIt =
          ...
          (useFlag ??
              (serviceDesc.nameInt == 'ala_bie' ||
                  ...
                  serviceDesc.nameInt == nameindexer));
    ...
    // Hosts that run their services inside a docker cluster instead of directly
    // on the VM. The generated yo-rc only states *which* hosts are compose/swarm
    // hosts (`LA_docker_compose_hostname`), never which services live in the
    // cluster: that is implicit, a docker host runs its whole assignment there.
    // Without this, an inventory generated by la-docker-compose came back as a
    // plain VM project with an empty cluster, i.e. the docker leg was lost on
    // every import.
    final Map<String, DeploymentType> dockerHosts = <String, DeploymentType>{};
    for (final MapEntry<String, DeploymentType> entry
        in <String, DeploymentType>{
          'docker_compose_hostname': DeploymentType.dockerCompose,
          'docker_swarm_hostname': DeploymentType.dockerSwarm,
        }.entries) {
      final String hosts = a(entry.key) as String? ?? '';
      for (final String hostname in hosts.split(RegExp(r'[, ]+'))) {
        if (hostname.isNotEmpty) {
          dockerHosts[hostname] = entry.value;
        }
      }
    }
    ...
    for (final LAServer server in p.servers) {
      ...
      // validateDataIntegrity), and so does anything the cluster cannot host.
      final List<String> clusterCandidates = assigned
          .where(
            (String s) =>
                s != dockerCompose &&
                s != dockerSwarm &&
                LAServiceDesc.listDockerCapableS.contains(s),
          )
          .toList();
      // The cluster pulls its sub-services in (pipelines drags spark, hadoop and
      // airflow along), so resolve that first: whatever the cluster ends up
      // running must not stay on the VM too, or invariant 1 breaks.
      final HashSet<String> clusterExpanded = _addSubServices(
        HashSet<String>.from(clusterCandidates),
      );
      final List<String> vmServices = assigned
          .where((String s) => !clusterExpanded.contains(s))
          .toList();
  ...
  List<String> validateDataIntegrity({bool checkParent = true}) {
    ...
    // Check 5: Docker clusters must have a carrier host. The UI walks users
    // into this (create server -> tick "docker compose" -> a cluster appears),
    // and a cluster without a machine under it is not deployable (gh-25).
    for (final LACluster cluster in clusters.where(
      (LACluster c) =>
          c.type == DeploymentType.dockerCompose ||
          c.type == DeploymentType.dockerSwarm,
    )) {
      if (cluster.serverId == null || cluster.serverId!.isEmpty) {
        errors.add(
          'Cluster "${cluster.name}" (${cluster.type.name}) has no carrier host: assign it to a server',
        );
      } else if (!servers.any((LAServer s) => s.id == cluster.serverId)) {
        errors.add(
          'Cluster "${cluster.name}" (${cluster.type.name}) points to a server (${cluster.serverId}) that does not exist',
        );
      }
    }
