...
class Api {
  static Future<String> genCasKey(int size) async {
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/cas-gen/$size');
  ...
  static Future<void> genSshConf(LAProject project, [bool forceRoot = false]) async {
    if (AppUtils.isDemo()) {
      return;
    }
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/gen-ssh-conf');
    final List<Map<String, dynamic>> servers = project.toJson()['servers'] as List<Map<String, dynamic>>;
  ...
  static Future<List<SshKey>> sshKeysScan() async {
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/ssh-key-scan');
    final Response response = await http.get(url);
    if (response.statusCode == 200) {
      final List<dynamic> l = (json.decode(response.body) as Map<String, dynamic>)['keys'] as List<dynamic>;
  ...
  static Future<void> genSshKey(String name) async {
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/ssh-key-gen/$name');
    http
        .get(url)
  ...
  static Future<Map<String, dynamic>> testConnectivity(List<LAServer> servers) async {
    if (AppUtils.isDemo()) {
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/test-connectivity');
    try {
      final Response response = await http.post(url,
          headers: <String, String>{'Content-type': 'application/json'},
      ...
      if (response.statusCode == 200) {
        final Map<String, dynamic> l = json.decode(response.body) as Map<String, dynamic>;
        return l;
      } else {
  ...
  static Future<void> importSshKey(String name, String publicKey, String privateKey) async {
    if (AppUtils.isDemo()) {
      return;
    }
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/ssh-key-import');
    final Response response = await http.post(url,
        headers: <String, String>{'Content-type': 'application/json'},
  ...
  static Future<void> saveConf(AppState state) async {
    final Map<String, dynamic> map = <String, dynamic>{
      for (final LAProject item in state.projects) item.id: item.toGeneratorJson()
    };
    if (AppUtils.isDemo()) {
      return;
    }
    ...
    try {
      final Response response = await http.post(url,
          headers: <String, String>{'Content-type': 'application/json'}, body: utf8.encode(json.encode(stateJ)));
      if (response.statusCode == 200) {
        return;
      } else {
        throw Exception('Failed to save configuration (${response.reasonPhrase}))');
      }
    } catch (e) {
      log(e.toString());
  ...
  static List<dynamic> retrieveProjectList(http.Response response) {
    final Map<String, dynamic> stateRetrievedJ = json.decode(response.body) as Map<String, dynamic>;
    return stateRetrievedJ['projects'] as List<dynamic>;
  }

  static Future<void> alaInstallSelect(String version, Function(String) onError) async {
    const String userError = 'Error selecting that ala-install version';
    if (AppUtils.isDemo()) {
      return;
    }
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/ala-install-select/$version');
    await http
        .get(url)
        .then((http.Response response) => response.statusCode != 200 ? onError(userError) : <String, dynamic>{})
  ...
  static Future<void> generatorSelect(String version, Function(String) onError) async {
    if (AppUtils.isDemo()) {
      return;
    }
    const String userError = 'Error installing that generator version';
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/generator-select/$version');
    await http
        .get(url)
        .then((http.Response response) => response.statusCode != 200 ? onError(userError) : <String, dynamic>{})
        .catchError((dynamic error) {
  ...
  static Future<void> regenerateInv({required LAProject project, required Function(String) onError}) async {
    if (AppUtils.isDemo()) {
      return;
    }
    final String id = project.isHub ? project.parent!.id : project.id;
    const String userError = 'Error generating your inventories';
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/gen/$id/false');
    http
        .get(url)
        .then((http.Response response) => response.statusCode != 200 ? onError(userError) : <String, dynamic>{})
        .catchError((dynamic error) {
  ...
  static Future<void> term(
    ...
    if (server != null && projectId != null) {
      body = utf8.encode(json.encode(<String, String>{'id': projectId, 'server': server}));
    }
    http
        .post(url, headers: <String, String>{'Content-type': 'application/json'}, body: body)
  ...
  static Future<void> ansiblew(DeployProject action) async {
    ...
    cmdTr.deployServices = cmdTr.deployServices.map((String name) => name == speciesLists ? 'lists' : name).toList();
    doCmd(url: url, projectId: action.project.id, desc: desc, cmd: cmdTr.toJson(), action: action);
  }
  ...
  static void doCmd(
    ...
    http
        .post(url,
            headers: <String, String>{'Content-type': 'application/json'},
            body: utf8.encode(json.encode(<String, Object>{'id': projectId, 'desc': desc, 'cmd': cmd})))
        .then((http.Response response) {
      if (response.statusCode == 200) {
        final Map<String, dynamic> l = json.decode(response.body) as Map<String, dynamic>;
        action.onStart(
            CmdHistoryEntry.fromJson(l['cmdEntry'] as Map<String, dynamic>), l['port'] as int, l['ttydPid'] as int);
      } else {
  ...
  static Future<CmdHistoryDetails?> getCmdResults(
      {required String cmdHistoryEntryId, required String logsPrefix, required String logsSuffix}) async {
    if (AppUtils.isDemo()) {
      return null;
    }
    ...
    if (response.statusCode == 200) {
      return CmdHistoryDetails.fromJson(json.decode(response.body) as Map<String, dynamic>);
    }
    return null;
  }

  static Future<String?> checkDirName({required String dirName, required String id}) async {
    if (AppUtils.isDemo()) {
      return null;
    }
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/check-dir-name');
    final Response response = await http.post(url,
        headers: <String, String>{'Content-type': 'application/json'},
        body: utf8.encode(json.encode(<String, String>{'dirName': dirName, 'id': id})));
    if (response.statusCode == 200) {
      return (json.decode(response.body) as Map<String, dynamic>)['dirName'] as String?;
    }
    return null;
  }
  ...
  static Future<String?> getBackendVersion() async {
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/get-backend-version');
    final Response response = await http.get(url);
    if (response.statusCode == 200) {
      return (json.decode(response.body) as Map<String, dynamic>)['version'] as String?;
    } else {
      return null;
    }
  }
  ...
  static Future<void> preDeploy(DeployProject action) async {
    ...
    doCmd(url: url, projectId: action.project.id, desc: action.cmd.desc, cmd: action.cmd.toJson(), action: action);
  }
  ...
  static Future<Map<String, dynamic>> checkHostServices(
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/test-host-services');
    final Response response = await http.post(url,
        headers: <String, String>{'Content-type': 'application/json'},
        body: utf8.encode(json.encode(<String, Object>{
          'projectId': projectId,
  ...
  static Future<Map<String, dynamic>> checkHostServicesWithStreaming(
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/test-host-services-stream');
    final Response response = await http.post(url,
        headers: <String, String>{'Content-type': 'application/json'},
        body: utf8.encode(json.encode(<String, Object>{
          'projectId': projectId,
  ...
  static Future<List<dynamic>> updateProject({required LAProject project}) async {
    return addOrUpdateProject(project, 'update');
  }

  static Future<List<dynamic>> addOrUpdateProject(LAProject project, String op) async {
    if (AppUtils.isDemo()) {
    ...
    final Response response = await (op == 'add' ? http.post : http.patch)(url,
        headers: <String, String>{'Content-type': 'application/json'}, body: utf8.encode(json.encode(body)));
    if (response.statusCode == 200) {
      return retrieveProjectList(response);
    } else {
  ...
  static Future<List<dynamic>> addProjects(List<LAProject> projects) async {
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/add-projects');
  ...
  static Future<List<dynamic>> deleteProject({required LAProject project}) async {
    if (AppUtils.isDemo()) {
      return <dynamic>[];
    }
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/delete-project');
  ...
  static void deployBranding(BrandingDeploy action) {
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/branding-deploy');
    doCmd(url: url, projectId: action.project.id, desc: action.cmd.desc, cmd: action.cmd.toJson(), action: action);
  }
  ...
  static void pipelinesRun(PipelinesRun action) {
    ...
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/pipelines-run');
    doCmd(url: url, projectId: action.project.id, desc: action.cmd.desc, cmd: action.cmd.toJson(), action: action);
  }

  static Future<Map<String, String>> getServiceDetailsForVersionCheck(LAProject project) async {
    final Map<String, dynamic> services = project.getServiceDetailsForVersionCheck();
    if (AppUtils.isDemo()) {
      return <String, String>{};
    }
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/get-service-versions');
    final Response response = await http.post(url,
        headers: <String, String>{'Content-type': 'application/json'},
    ...
    if (response.statusCode == 200) {
      ...
      final Map<String, String> versions =
          l.map((String key, dynamic value) => MapEntry<String, String>(key, value.toString()));
  ...
  static Future<String> fetchDependencies() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to retrieve dependencies (${response.statusCode})');
    }
  }

  static Future<Map<String, dynamic>> solrQuery(String projectId, String solrHost, String query) async {
    if (AppUtils.isDemo()) {
      return <String, dynamic>{};
    }
    ...
    try {
      ...
      final Response response = await http.post(url,
          headers: <String, String>{'Content-type': 'application/json'},
          body: utf8.encode(json.encode(<String, Object>{'id': projectId, 'sshHost': solrHost, 'query': query})));
      if (response.statusCode == 200) {
        final Map<String, dynamic> jsonResponse = jsonDecode(response.body) as Map<String, dynamic>;
        return jsonResponse;
      } else {
        throw Exception("Failed to query solr '$query' (${response.reasonPhrase}))");
      }
    } catch (e) {
  ...
  static Future<dynamic> solrRawQuery(String projectId, String solrHost, String query) async {
    if (AppUtils.isDemo()) {
      return <String, dynamic>{};
    }
    final Uri url = AppUtils.uri(dotenv.env['BACKEND']!, '/api/v1/solr-query');
  ...
  static Future<dynamic> mySqlQuery(String projectId, String mysqlHost, String db, String query) async {
    if (AppUtils.isDemo()) {
      return <String, dynamic>{};
    }
    ...
    try {
      final Response response = await http.post(url,
          headers: <String, String>{'Content-type': 'application/json'},
          body: utf8
              .encode(json.encode(<String, Object>{'id': projectId, 'sshHost': mysqlHost, 'db': db, 'query': query})));
      if (response.statusCode == 200) {
