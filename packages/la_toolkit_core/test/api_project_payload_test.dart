...
void main() {
  ...
  test('a freshly created project carries its defaulted variables', () {
    ...
    final Map<String, dynamic> body = p.toApiJson();

    final List<dynamic> sent = body['variables'] as List<dynamic>;
    expect(sent, isNotEmpty);
    expect(
      sent,
      hasLength(p.variables.length),
  ...
  test('the same ids are sent on the next save', () {
    final LAProject p = minimalPortal();
    final List<dynamic> first = p.toApiJson()['variables'] as List<dynamic>;
    final List<dynamic> second = p.toApiJson()['variables'] as List<dynamic>;
    List<String> ids(List<dynamic> l) =>
        l
            .map((dynamic v) => (v as Map<String, dynamic>)['id'] as String)
            .toList()
          ..sort();
